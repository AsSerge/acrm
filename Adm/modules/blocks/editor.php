<?php
/**
 * Модуль Blocks - файловый менеджер + редактор кода
...
<?php if (!$showEditor): ?>
    ...
    <div class="module-editor">
        <div class="editor-header">
            ...
            <a href="?route=blocks" class="btn-admin btn-admin-secondary">← Назад к блокам</a>
        </div>
    ...
    $(document).ready(function() {
        ...
        function renderFiles(files, path) {
            ...
            fileItems.forEach(function(item) {
                html += '<div class="file-item">';
                html += '<a href="?route=blocks&action=editor&file=' + encodeURIComponent(item.path) + '" class="file-item-left">';
                html += '<span class="file-icon">' + iconFile + '</span>';
                html += '<span>' + item.name + '</span>';
                html += '<span class="file-meta">' + (item.size || 0) + ' B · ' + (item.modified || '') + '</span>';
                html += '</a>';
                html += '<div class="file-item-right">';
                html += '<a href="?route=blocks&action=editor&file=' + encodeURIComponent(item.path) + '" class="file-action-btn" title="Редактировать">' + iconEdit + '</a>';
                html += '<button type="button" class="file-action-btn rename" data-rename="' + item.path + '" data-name="' + item.name + '" title="Переименовать">' + iconRename + '</button>';
                html += '<button type="button" class="file-action-btn delete" data-delete="' + item.path + '" data-name="' + item.name + '" title="Удалить">' + iconDelete + '</button>';
                html += '</div>';
                html += '</div>';
        ...
        $('#btn-create-file').on('click', function() {
            ...
            $('#modal-input').attr('placeholder', 'например: block.php').val('');
        ...
        $('#btn-create-folder').on('click', function() {
            ...
            $('#modal-input').attr('placeholder', 'например: images').val('');
            $('#modal-confirm').text('Создать');
            $('#modal-overlay').addClass('active');
            $('#modal-input').focus();
        });
    ...
    <div class="module-editor">
        <div class="editor-header">
            ...
            <div class="editor-actions">
                ...
                <a href="?route=blocks&action=editor" class="btn-admin btn-admin-secondary">← К списку файлов</a>
            </div>
        </div>
